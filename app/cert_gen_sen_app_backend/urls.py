...
urlpatterns = [
    ...
    path("upload-participant-image/<int:pk>", UploadParticipantImageView.as_view()),
    path("upload-event-album/<slug:slug>", ParticipantImageAlbumView.as_view()),
    path("generate-certificate/<slug:slug>", GenerateCertificate.as_view()),
    path(
        "generate-certificate/<slug:slug>/<int:pk>", GenerateCertificateById.as_view()
    ),
    path("preview-certificate/", Ppt2Image.as_view()),
    path("upload-completion-template/", UploadCompletionTemplateView.as_view()),
    path("upload-merit-template/", UploadMeritTemplateView.as_view()),
    path("contribute-completion-template/", ContributeCompletionView.as_view()),
    path("contribute-merit-template/", ContributeMeritView.as_view()),
]
